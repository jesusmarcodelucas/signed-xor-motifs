/*
 * randomize_celegans.c
 *
 * C. elegans adaptation of the Roberts & Coolen (Phys. Rev. E 85, 046103, 2012)
 * degree-preserving edge-swap randomizer.
 *
 * C. elegans CSV format (8 columns, semicolon-separated, header optional):
 *   Neuron1;Neuron2;Weight;Neurotransmitter;Classification;REF;E/I original;Check
 * where Neurotransmitter in {GLU, ACH, GABA, ...}
 * and   Classification   in {EXC, INH}
 *
 * Self-loops (Neuron1 == Neuron2) are silently skipped.
 *
 * Two operating modes:
 *
 *   --by-nt   (default, biologically correct)
 *       Each distinct Neurotransmitter is its own layer. GLU edges only swap
 *       with GLU edges, GABA with GABA, ACH with ACH, etc.
 *       Preserves Dale's principle at the NT level.
 *
 *   --lump-inhibitory
 *       Two layers: EXC (GLU + ACH) and INH (GABA), using the Classification
 *       column. Allows cross-NT swaps within the excitatory pool (GLU<->ACH),
 *       which is methodologically consistent with the mouse/Drosophila null.
 *
 *   --include-other
 *       By default, edges with NT not in {GLU, ACH, GABA} are preserved
 *       as-is. Pass this flag to also randomize them within their own layers.
 *
 * All 8 original columns are faithfully round-tripped to the output file.
 * Only Neuron2 (post-synaptic target) changes; all other fields are preserved.
 *
 * Acceptance: accept-all (Roberts/Coolen Sec. VI.C).
 *
 * Compile:
 *   gcc -O3 -march=native -o randomize_celegans randomize_celegans.c
 *
 * Usage:
 *   ./randomize_celegans in.csv --out out.csv --seed N \
...
typedef struct {
    ...
    char     nt[MAX_NT_LEN];
    char     col2[16];   /* Weight        — preserved verbatim */
    char     col3[64];   /* Neurotransmitter */
    char     col5[64];   /* REF           — preserved verbatim */
    char     col6[8];    /* E/I original  — preserved verbatim */
    char     col7[32];   /* Check         — preserved verbatim */
} all_edge_t;
...
static void all_push(int s, int t, layer_t *L, size_t idx,
    ...
    strncpy(all_edges[all_n].col2, c2, 15);  all_edges[all_n].col2[15]=0;
    strncpy(all_edges[all_n].col3, c3, 63);  all_edges[all_n].col3[63]=0;
    strncpy(all_edges[all_n].col5, c5, 63);  all_edges[all_n].col5[63]=0;
    strncpy(all_edges[all_n].col6, c6, 7);   all_edges[all_n].col6[7]=0;
    strncpy(all_edges[all_n].col7, c7, 31);  all_edges[all_n].col7[31]=0;
    all_n++;
}
...
static int split_semi(char *line, char *out[], int max_fields){
    int k=0; char *p=line;
    while(k<max_fields){
        out[k++]=p;
        char *c=strchr(p,';');
...
/* Decide which layer name an edge belongs to under current mode.
   For C. elegans: GLU and ACH are excitatory, GABA is inhibitory.
   The classification column (cls) is used for LUMP_INH mode. */
static const char *layer_for_nt(const char *nt, const char *cls){
    static const char *EXC = "EXC", *INH = "INH";
    if(mode == MODE_LUMP_INH){
        /* Use Classification column directly */
        if(!strcmp(cls,"EXC")) return EXC;
        if(!strcmp(cls,"INH")) return INH;
        return include_other ? nt : NULL;
    } else {
        /* MODE_BY_NT: each NT is its own layer */
        if(!strcmp(nt,"GLU")||!strcmp(nt,"ACH")||!strcmp(nt,"GABA")) return nt;
        return include_other ? nt : NULL;
    }
}
...
static void read_csv(const char *path){
    ...
    while(fgets(line,sizeof(line),fp)){
        ...
        if(!line[0]) continue;
        char *fields[9];
        int nf=split_semi(line,fields,9);
        if(nf<5) continue;

        char *pre  = trim_field(fields[0]);  /* Neuron1        */
        char *post = trim_field(fields[1]);  /* Neuron2        */
        char *c2   = trim_field(fields[2]);  /* Weight         */
        char *nt   = trim_field(fields[3]);  /* Neurotransmitter */
        char *cls  = trim_field(fields[4]);  /* Classification */
        char *c5   = nf>5 ? trim_field(fields[5]) : "";  /* REF */
        char *c6   = nf>6 ? trim_field(fields[6]) : "";  /* E/I original */
        char *c7   = nf>7 ? trim_field(fields[7]) : "";  /* Check */

        /* Skip self-loops */
        if(strcmp(pre,post)==0) continue;

        int sid = intern_id(pre);
        int tid = intern_id(post);

        const char *lname = layer_for_nt(nt, cls);
        if(!lname){
            all_push(sid, tid, NULL, 0, nt, c2, nt, c5, c6, c7);
            continue;
        }
        layer_t *L = get_layer(lname, 1);
        size_t idx = L->E.n;
        el_push(&L->E, sid, tid);
        all_push(sid, tid, L, idx, nt, c2, nt, c5, c6, c7);
    }
    fclose(fp);
}
...
static void write_csv(const char *path){
    ...
    for(size_t i=0;i<all_n;i++){
        ...
        /* Reconstruct Classification from NT for --by-nt mode output */
        const char *cls;
        if(!strcmp(e->col3,"GLU")||!strcmp(e->col3,"ACH")) cls="EXC";
        else if(!strcmp(e->col3,"GABA"))                   cls="INH";
        else                                                cls="other";
        fprintf(fp, "%s;%s;%s;%s;%s;%s;%s;%s\n",
                id_to_name[s], id_to_name[d],
                e->col2, e->col3, cls,
                e->col5, e->col6, e->col7);
    }
    fclose(fp);
}
