/*
 * celegans_signed_xor_motif.c
 *
 * Adapted from drosophila_signed_xor_motif.c for the C. elegans connectome.
 *
 * INPUT FORMAT (tab-separated, with header):
 *   Neuron1 <TAB> Neuron2 <TAB> Weight <TAB> Neurotransmitter <TAB> Classification <TAB> REF <TAB> E/I original <TAB> Check
 *
 * EDGE CLASSIFICATION:
 *   Uses the "Classification" column (col 4, 0-indexed) which contains "EXC" or "INH".
 *   Falls back to Neurotransmitter (col 3): ACH/GLU -> EXC, GABA -> INH.
 *   Self-loops (Neuron1 == Neuron2) are silently skipped.
 *
 * SIGNED-XOR MOTIF (8 nodes, 12 edges):
 *
 * BASE XOR (8 edges):
 ...
 *      (5, F+)  with sign matching node 5's identity
 *      (5, F-)  with sign matching node 5's identity
 *      AND either Variant A OR Variant B:
 *          Variant A: (1, F+) EXC  AND  (3, F-) EXC
 *          Variant B: (2, F+) EXC  AND  (4, F-) EXC
 *      A motif is counted if A OR B holds.
 *
 * NEURON IDENTITY:
 *      F+ must be excitatory (only EXC outgoing edges).
 *      F- must be inhibitory (only INH outgoing edges).
 *      Mixed neurons (both EXC and INH outputs) are skipped.
 *
 * DISTINCTNESS:
 *      All 8 neurons must be pairwise distinct.
 *
 * Compile:
 *      gcc -O3 -march=native -fopenmp -o celegans_signed_xor_motif \
 *          celegans_signed_xor_motif.c
 *
 * Usage:
 *      ./celegans_signed_xor_motif input.csv [--out motifs.csv] [--induced] [-q]
 *
 * Output CSV (if --out) has 9 columns:
 *      n1,n2,n3,n4,n5,n6,fp,fm,variants   (variants in {A,B,AB})
 */
...
/*
 * classify_nt: C. elegans uses ACH (EXC), GLU (EXC), GABA (INH).
 * Also accepts the explicit "EXC"/"INH" strings from the Classification column.
 */
static edgeclass_t classify_nt(const char *nt) {
    if (!nt) return EDGE_OTHER;
    while (*nt && (isspace((unsigned char)*nt) || *nt == '"')) nt++;
    /* Explicit classification column values */
    if (!strncmp(nt, "EXC",  3)) return EDGE_EXC;
    if (!strncmp(nt, "INH",  3)) return EDGE_INH;
    /* Neurotransmitter fallback */
    if (!strncmp(nt, "ACH",  3)) return EDGE_EXC;
    if (!strncmp(nt, "GLU",  3)) return EDGE_EXC;
...
/* C. elegans data is semicolon-separated */
static int split_tsv(char *line, char *out[], int max_fields) {
    int k = 0; char *p = line;
    while (k < max_fields) {
        out[k++] = p;
...
static csv_stats_t read_csv(const char *path) {
    ...
    if (fgets(line, sizeof(line), fp)) {
        rstrip(line);
        /* Skip header if first field is non-numeric (e.g. "Neuron1") */
        ...
        if (!looks_numeric && first_field[0]) {
            /* header row — skip it, stay at current position */
        } else {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (!line[0]) continue;
        char *fields[8];
        int nf = split_tsv(line, fields, 8);
        if (nf < 5) continue;

        const char *pre   = fields[0];   /* Neuron1        */
        const char *post  = fields[1];   /* Neuron2        */
        /* fields[2] = Weight  (unused) */
        /* fields[3] = Neurotransmitter (fallback) */
        const char *cls_s = fields[4];   /* Classification: "EXC" or "INH" */
        ...
        edgeclass_t cls = classify_nt(cls_s);
...
int main(int argc, char **argv) {
    ...
    if (!csv) {
        fprintf(stderr, "Usage: %s input.tsv [--out motifs.csv] [--induced] [-q]\n", argv[0]);
        return 1;
    }
